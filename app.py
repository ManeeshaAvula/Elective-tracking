...
def get_students_by_branch(branch):
    try:
        ...
        cursor = connection.cursor()
        query = f"SELECT Name, Reg_Id, SUB_NAME, Dept, Year FROM student_details2 WHERE Dept = '{branch}'"
        cursor.execute(query)
        results = cursor.fetchall()
        cursor.close()
        connection.close()
...
@app.route('/students/delete_elective', methods=['POST'])
def delete_elective():
    try:
        data = request.json
        connection = mysql.connector.connect(**db_config)
        ...
        delete_query = f"""
            ...
            WHERE Reg_Id = '{data['Reg_Id']}' AND SUB_CODE = '{data['SUB_CODE']}' AND SUB_NAME = '{data['SUB_NAME']}' AND Sem = '{data['Sem']}'
        """
